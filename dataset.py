...
class EMGTestDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        snr = noisy_file_name.split(os.sep)[-3]
        noise_id = noisy_file_name.split(os.sep)[-2]
        # data = np.vstack((clean_data, noisy_data))
        return Tensor(clean_data).unsqueeze(0), Tensor(noisy_data).unsqueeze(0), snr, Tensor(clean_sti).unsqueeze(0), os.path.basename(noisy_file_name).replace('.npy', ''), noise_id
